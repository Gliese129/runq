...
func TestDataDirOverride(t *testing.T) {
	...
	if got, want := config.ConfigPath(), filepath.Join(dir, "config.yaml"); got != want {
...
func TestWriteTemplateThenLoad(t *testing.T) {
	...
	if path != config.ConfigPath() {
		t.Fatalf("path = %q, want %q", path, config.ConfigPath())
	}
...
// WriteTemplate appends hpc: to an existing config that only has global keys.
func TestWriteTemplateAppendsToExistingGlobal(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("RUNQ_DATA_DIR", dir)

	// Pre-create a config with only global keys.
	existing := "data_path: \"/scratch/runq\"\n"
	if err := os.WriteFile(filepath.Join(dir, "config.yaml"), []byte(existing), 0o644); err != nil {
		t.Fatal(err)
	}

	path, created, err := WriteTemplate("slurm")
	if err != nil {
		t.Fatalf("WriteTemplate: %v", err)
	}
	if !created {
		t.Fatal("expected created=true when appending hpc section")
	}

	// The file should now parse with both global and hpc sections.
	cfg, err := Load()
	if err != nil {
		t.Fatalf("Load after append: %v", err)
	}
	if cfg.SubmitTemplate == "" {
		t.Error("hpc section not appended")
	}

	// Global keys should be preserved.
	buf, _ := os.ReadFile(path)
	if !strings.Contains(string(buf), "data_path") {
		t.Error("global data_path lost after append")
	}
}

// WriteTemplate refuses to overwrite an existing hpc section.
func TestWriteTemplateSkipsExistingHPC(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("RUNQ_DATA_DIR", dir)
	...
	_, created, err := WriteTemplate("pbs")
	if err != nil {
		t.Fatalf("WriteTemplate: %v", err)
	}
	if created {
		t.Error("expected created=false when hpc: already exists")
	}

	// Should still be slurm, not pbs.
	cfg, _ := Load()
	if !strings.Contains(cfg.StatusTemplate, "sacct") {
		t.Error("hpc section was overwritten")
	}
}

// Global keys (data_path) should not interfere with HPC loading.
func TestLoadIgnoresGlobalSection(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("RUNQ_DATA_DIR", dir)
	body := `data_path: "/scratch/runq"
hpc:
...
`
	...
	if cfg.SubmitTemplate != "sbatch {{run_sh}}" {
		t.Errorf("unexpected submit_template: %q", cfg.SubmitTemplate)
	}
}
