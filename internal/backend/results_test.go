...
func TestResultsMultiModelTaskSplitsRuns(t *testing.T) {
	// One eval task recording two models: identity clustering wins, the
	// task appears as one contiguous run per identity group.
	st := seedResultsStore(t, "t1")
	insertRecords(t, st, "t1", [][2]string{
		{`{"model":"a","step":1}`, `{"m":1}`},
		{`{"model":"b","step":1}`, `{"m":2}`},
	...
	// Same task id, but contiguous runs merge across the group boundary:
	// records are all t1 and adjacent → a single 3-record run is correct.
	if len(res.Schema.Tasks) != 1 || res.Schema.Tasks[0].Count != 3 {
		t.Errorf("tasks = %+v", res.Schema.Tasks)
	}
}
